-- Fresh schema for the current frontend and app tables.
...
create table if not exists public.daily_logs (
  ...
  mood text not null default 'neutral' check (mood in ('low', 'neutral', 'good')),
  day_rating smallint not null default 7 check (day_rating between 1 and 10),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
create table if not exists public.goals (
  ...
  time_horizon text not null default 'weekly' check (time_horizon in ('daily', 'weekly', 'long-term')),
